...
function plot_joint_move_results(t, q, qd, qdd, cart_pos, cart_vel, cart_acc, A, B, C)
    ...
    % 定義計算置中位置的輔助函式: [left, bottom, width, height]
    ...
    figure('Name', 'Joint Move - Joint Velocities', 'NumberTitle', 'off', ...
           'Position', get_pos(1000, 700));
    for i = 1:6
        subplot(3, 2, i);
    ...
    figure('Name', 'Joint Move - Joint Accelerations', 'NumberTitle', 'off', ...
           'Position', get_pos(1000, 700));
    for i = 1:6
        subplot(3, 2, i);
    ...
    %% 視窗 4: 末端執行器 (End-effector)
    figure('Name', 'Joint Move - Cartesian Analysis', 'NumberTitle', 'off', ...
